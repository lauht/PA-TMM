...
from tab_network import TabNetEncoder # https://github.com/dreamquark-ai/tabnet/blob/develop/pytorch_tabnet/tab_network.py
...
class PATMM(nn.Module):
    def __init__(self, n_feat, pretrained_path=None, dropout=0.2, n_out=2):
        ...
        self.rnn = PriceEmbedding(n_feat=n_feat-24, out_dim=dp, rnn_layers=2, dropout=dropout)
        self.TabNetEncoder = TabNetEncoder(input_dim=24,output_dim=dq) # Please refer to https://github.com/dreamquark-ai/tabnet for the source code of TabNet.
        self.Wum = nn.Parameter(torch.Tensor(dm+1, dr))
        self.Wvm = nn.Parameter(torch.Tensor(dm+1, dr))
        self.Wup = nn.Parameter(torch.Tensor(dp+dq+1, dr))
        self.Wvp = nn.Parameter(torch.Tensor(dp+dq+1, dr))
    ...
    def forward(self, x, x_message): 
        ...
        u_p = self.leaky_relu(torch.cat([pq, activation_status], dim=1).matmul(self.Wup)) 
        v_p = self.leaky_relu(torch.cat([pq, activation_status], dim=1).matmul(self.Wvp)) 
...
class MPA(nn.Module):
    def __init__(self, n_feat, pretrained_path=None, dropout=0.2, n_out=2):
        ...
        self.Wum = nn.Parameter(torch.Tensor(dm+1, dr))
        self.Wvm = nn.Parameter(torch.Tensor(dm+1, dr))
        self.Wup = nn.Parameter(torch.Tensor(dp+dq, dr))
        self.Wvp = nn.Parameter(torch.Tensor(dp+dq, dr))
    ...
    def forward(self, x, x_tag): 
        ...
        # u_m = self.leaky_relu(torch.cat([m, activation_status], dim=1).matmul(self.Wum))
        v_m = self.leaky_relu(torch.cat([m, activation_status], dim=1).matmul(self.Wvm))
        u_p = self.leaky_relu(pq.matmul(self.Wup)) 
        # v_p = self.leaky_relu(pq.matmul(self.Wvp))
